...
An incremental integration can first import every visible record as a raster layer while retaining all opaque blocks. A second phase can map groups and masks, followed by editable text, shapes, adjustments, and layer effects. The retained blocks keep unsupported features available for later semantic support and reduce destructive round trips during that rollout.
